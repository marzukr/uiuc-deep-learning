...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250, 
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            D.train()
            D_solver.zero_grad()
            z1 = sample_noise(batch_size, noise_size).to(device)
            fake_images = G(z1).reshape(batch_size, input_channels, img_size, img_size)

            logits_real = D(real_images).reshape(batch_size,1)
            logits_fake = D(fake_images).reshape(batch_size,1)
            ...
            G.train()
            G_solver.zero_grad()
            z2 = sample_noise(batch_size, noise_size).to(device)
            fake_images_g = G(z2).reshape(batch_size, input_channels, img_size, img_size)

            logits_fake_g = D(fake_images_g).reshape(batch_size,1)
